...
const options = { plugins: ["@babel/plugin-syntax-jsx", plugin] }
...
it("handles basic (copied) layout props", () => {
  ...
  expect(code).toBe(
    "<span css={`" +
      defaultCss +
      "font-family: Arial;color: red;font-size: 20px;letter-spacing: 20px;text-decoration: underline;font-weight: 100;text-transform: capitalize;`} />;"
  )
})
...
it("handles italic prop identifier", () => {
  ...
  expect(code).toBe(
    "<span css={`" +
      defaultCss +
      '${someVar === true ? "font-style: italic;" : ""}`} />;'
  )
})
...
it("handles uppercase prop", () => {
  ...
  expect(code).toBe(
    "<span css={`" + defaultCss + "text-transform: uppercase;`} />;"
  )
})
